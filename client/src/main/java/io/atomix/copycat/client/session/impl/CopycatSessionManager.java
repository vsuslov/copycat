...
import io.atomix.copycat.client.util.ConnectionManager;
...
public class CopycatSessionManager {
  ...
  public CopycatSessionManager(CopycatClientState clientState, ConnectionManager connectionManager, ThreadContext threadContext, ScheduledExecutorService threadPoolExecutor, ConnectionStrategy connectionStrategy, Duration sessionTimeout, Duration unstableTimeout) {
    ...
    this.connection = new ClientConnection(new CopycatSessionState(0, "", ""), connectionManager, selectorManager.createSelector(CommunicationStrategies.ANY));
  ...
  public CompletableFuture<Void> open() {
    ...
    threadContext.execute(() -> registerClient(new RegisterAttempt(1, future)));
    return future;
  }
   ...
   * @param selectionStrategy The strategy with which to communicate with servers.
   * @return A completable future to be completed once the session has been opened.
   */
  public CompletableFuture<CopycatSession> openSession(String name, String type, CommunicationStrategy selectionStrategy) {
    ...
    Runnable callback = () -> connection.<OpenSessionRequest, OpenSessionResponse>sendAndReceive(OpenSessionRequest.NAME, request).whenCompleteAsync((response, error) -> {
      if (error == null) {
        if (response.status() == Response.Status.OK) {
          ...
          ClientConnection connection = new ClientConnection(state, connectionManager, selectorManager.createSelector(selectionStrategy));
          future.complete(new DefaultCopycatSession(state, connection, threadContext, this));
        } else {
